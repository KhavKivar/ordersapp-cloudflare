...
export default function ProductEditPage() {
  ...
  useEffect(() => {
    ...
    reset({
      name: product.name,
      type: product.type as CreateProductDto["type"],
      sizeMl: product.sizeMl ?? undefined,
      sellPriceClient: product.sellPriceClient,
      buyPriceSupplier: product.buyPriceSupplier,
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (data: CreateProductDto) => updateProduct(productId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["products"] });
      navigate("/product", { state: { toast: "Producto actualizado" } });
    },
    onError: () => {
      ...
      <div className="mx-auto flex w-full max-w-md flex-col px-5 pt-6 pb-32">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div className="min-w-0">
            <h1 className="text-lg font-black text-foreground leading-tight truncate">Editar producto</h1>
            <p className="text-[11px] text-muted-foreground/60 font-medium truncate">{product.name}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit((data) => mutate(data))} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={cn(
                    "w-full rounded-2xl border bg-muted/30 px-3 py-4 pl-9 text-[15px] font-medium text-foreground focus:outline-none focus:ring-2 transition-all",
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              ...
              <input
                type="number"
                {...register("sizeMl", { setValueAs: (v) => (v === "" || isNaN(Number(v)) ? null : Number(v)) })}
                className="w-full rounded-2xl border border-border/50 bg-muted/30 px-4 py-4 text-[15px] font-medium text-foreground focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500/40 transition-all"
              />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] font-black uppercase tracking-[0.08em] text-muted-foreground/70">
